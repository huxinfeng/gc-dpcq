...
class ProjectClientSceneObject extends SceneObjectCommon {
    ...
    constructor(soData: SceneObject, scene: ClientScene) {
        ...
        this.settingX = this.x;
        this.settingY = this.y;
    }
    /**
     * 释放函数
     */
    ...
    battlerInit(refreshAttribute: boolean = true, fullState: boolean = true): void {
        ...
        // -- 刷新战斗者属性
        if (this.isBattler) {
            if (refreshAttribute) Game.refreshActorAttribute(this.actor, 1, this);
            this.battleAI = new GameBattleAI(this);
            // 已死亡的情况
            if (this.isDead) GameBattlerHandler.battlerDeadAnimation(this, false);
            else if (fullState) {
    ...
    refreshPointBar(): void {
        ...
        if (this.inPartyIndex >= 0) {
            var mainUI: GUI_Main = GameUI.get(13) as any;
    ...
    update(nowTime: number): void {
        ...
        // 刷新行为
        if (!this.isDead || !this.isBattler) this.updateBehavior();
        // 刷新并行事件
